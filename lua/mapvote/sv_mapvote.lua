...
function MapVote.Start(length, current, limit, prefix, callback)
    ...
    if #MapVote.AllowedMaps > 0 then
        local _maps = {}
        for iter, map in pairs(maps) do
            local mapName = map:sub(1, -5)
            if table.HasValue(MapVote.AllowedMaps, mapName) then
                _maps[iter] = map
            end
        end
        maps = _maps
    ...
    if #MapVote.BannedMaps > 0 then
       local _maps = {}
       for iter, map in pairs(maps) do
          local mapName = map:sub(1, -5)
          if table.HasValue(MapVote.BannedMaps, mapName) == false then
              _maps[iter] = map
          end
       end
       maps = _maps
